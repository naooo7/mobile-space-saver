...
function DrillPage() {
  ...
  return <Page narrow><PageTitle eyebrow="Build your session" title="Custom Drill" subtitle="Choose what to focus on. You can change any setting before starting." />
    <div className="space-y-4">
      ...
      <Surface><Step n="3" title="Choose Material" /><div className="grid gap-2 sm:grid-cols-2">{availableMaterials.map((item) => { const active = selected.includes(item.id); return <label key={item.id} className={cn("glass-pill flex cursor-pointer items-center gap-3 rounded-xl p-3.5", active && "glass-pill-active")}><Checkbox checked={active} onCheckedChange={() => toggle(item.id)} /><span className="text-sm font-medium">{item.name}</span></label>; })}</div></Surface>
      ...
      <Surface><Step n="6" title="60s Challenge" /><div className="flex items-start justify-between gap-6"><div><h3 className="font-semibold">Challenge Mode</h3><p className="mt-1 text-sm text-muted-foreground">60 seconds per question. A timeout counts as incorrect.</p></div><div className="glass-pill mt-1 rounded-full p-1"><Switch checked={challenge} onCheckedChange={setChallenge} aria-label="Challenge mode" /></div></div></Surface>
